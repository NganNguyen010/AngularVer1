...
export class MailServiceService {
  ...
  getAll(folder: string) {
    return this.messageSource = this.email.filter(x => x.folder === folder);
  }
  getDetail (id: string) {
    return this.detailSource = this.email.find(x => x._id === id);
  }
}
